 ...
 */
        // 08.07.2024
describe('This is first test suite, Nikita Novinkins', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('input[name="firstName"]').type('Nikita')
        cy.get('[data-testid="lastNameTestId"]').type('Novinkins')
        cy.get('input[name="password"]').type('Nikita123')
        cy.get('[name="confirm"]').type('Nikita123')
    ...
    */
    ...
    it('User cannot submit data when phone number is absent', () => {
        // Enter all other fields correctly
        cy.get('#username').type('johnDoe')
        cy.get('input[name="password"]').type('Password123')
        cy.get('[name="confirm"]').type('Password123')
    
        // Skip entering the phone number
        // cy.get('[data-testid="phoneNumberTestId"]').type('10203040') // This line is intentionally omitted
    
        // Attempt to submit the form by clicking a non-interactive element to trigger validation
        cy.get('h2').contains('Password').click()
        ...
        // Double-check the visibility of the error message using CSS properties
    ...
    it('User cannot submit data when password and/or confirmation password is absent', () => {
        ...
        // Skip entering the password and confirmation password
        // cy.get('input[name="password"]').type('Password123') // This line is intentionally omitted
        // cy.get('[name="confirm"]').type('Password123') // This line is intentionally omitted
    ...
    it('User cannot add letters to phone number', () => {
        // Enter the username, password, and confirmation password correctly
        cy.get('#username').type('johnDoe')
        cy.get('input[name="password"]').type('Password123')
        cy.get('[name="confirm"]').type('Password123')
    
        // Attempt to enter letters into the phone number input field
        cy.get('[data-testid="phoneNumberTestId"]').type('Cerebrum Hub')
    
        // Verify that the phone number input field has the type attribute set to 'number'
